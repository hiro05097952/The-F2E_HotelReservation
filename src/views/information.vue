<template>
    <div id="app" v-if="getOb">
      ...
      <ul class="infoL">
        ...
        <li>房間人數限制：{{ getOb.descriptionShort.GuestMin }}~{{ getOb.descriptionShort.GuestMax }} 人</li>
      ...
      <table class="iconWrap">
        ...
        <tr>
          <td :class="{'no' : !getOb.amenities['Room-Service']}"><i id="serviceIcon"></i>Room Service</td>
  ...
  computed: {
    ...
    getBooking () {
      ...
      for (let i = 0; i < this.$store.getters.infoOb.booking.length; i++) {
        newArr.push(this.$store.getters.infoOb.booking[i].date)
      }
      return newArr
    }
  },
  ...
  methods: {
    ...
    reservation () {
      ...
      if (!this.$children[0].checkIn || !this.$children[0].checkOut) {
        ...
        let checkInDt = new Date(this.$children[0].checkIn)
        let checkIn = this.converter(checkInDt)

        let checkOutDt = new Date(this.$children[0].checkOut)
        let checkOut = this.converter(checkOutDt)
        // 幾晚 + 天數陣列
        let nights = (checkOutDt - checkInDt) / (1000 * 60 * 60 * 24)
        let nightArr = []
        // 轉換日期格式 + 確認 平日 假日 總金額
        let holiday = 0
        let weekday = 0
        ...
        let total = this.getOb.normalDayPrice * weekday + this.getOb.holidayPrice * holiday
    ...
    converter (value) {
      ...
      let month = dt.getMonth() + 1 < 10 ? '0' + String(dt.getMonth() + 1) : dt.getMonth() + 1
      let day = dt.getDate() < 10 ? '0' + String(dt.getDate()) : dt.getDate()
...
<style lang="scss" src="@/sass/info.scss" scoped>
</style>
