    ...
    .datepicker( :class='`${ isOpen ? "datepicker--open" : "datepicker--closed" }`')
      .-hide-on-desktop
        .datepicker__dummy-wrapper.datepicker__dummy-wrapper--no-border(
          @click='toggleDatepicker' :class="`${isOpen ?
          ...
          .datepicker__input(
            tabindex="0"
            :class="`${isOpen && checkOut == null &&
            checkIn !== null ? 'datepicker__dummy-input--is-active' : ''}`"
...
/* eslint-disable */
  ...
  export default {
    ...
    methods: {
      ...
      getMonth(date) {
        return (this.showYear ? fecha.format(date, ' YYYY') : '') +` / `+ this.i18n["month-names"][fecha.format(date, 'M') - 1];
        ...
        &__month-day {
            ...
            &--selected {
              // 選取中間
                ...
                background: url('../assets/border_black.svg') center center no-repeat;
